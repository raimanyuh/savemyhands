...
A **mobile redesign** is in flight on `dev` (multi-PR, design lives in `design_handoff_mobile_redesign/`). The **mobile recorder is shipped** as of 2026-05-22 — `/record` below 640px now renders a sibling `MobileRecorder` (vertical PokerNow-style felt, fixed bottom action bar, full-screen / drawer raise sizer based on viewport height, content-height card-picker bottom sheet, gear-in-meta-strip setup sheet, showdown panel with per-villain show / muck). Dashboard, replayer, auth pages, and a separate landing-page redesign (in `design_handoff_landing_page/`) are still pending — see "Mobile redesign — in progress" below.
...
- **Mobile recorder** (`/record` below 640px) — sibling implementation under `components/mobile/`, branched at the route via `RecorderShell`. Vertical PokerNow-style felt: hero anchored bottom-center on a portrait ellipse, opponents fanned counter-clockwise, hand-tuned seat coordinates for 6/7/8-handed (cap at 8). Slim 48pt header (back / hand name + stakes subtitle / empty right slot). 36pt meta strip with a `⚙ Setup` chip on the left during setup, replaced by a `STREET` pill once the hand starts (settings are locked mid-hand so the gear disappears). Bottom region morphs by phase: `Pick hero hole cards (X/Y)` CTA → 3-button action bar with Undo + Step counter → showdown panel with per-villain Show/Muck rows → Save hand. Bet bubbles render in a separate absolute layer that interpolates each seat's coords toward the felt center via the same `chipXY` math the desktop uses. Pot-limit math, double board, bomb pot, PLO 4/5 all compose. Card picker is a content-height `<BottomSheet>` with suit-then-rank selection (44pt suit row, 56pt rank cells); auto-advances through multi-card rows for hero / villain / flop. Raise sizer is a full-screen modal at vh<700, an inline drawer above the action bar at vh≥700. Big amount field is a typeable `inputMode="numeric"` input that opens the numpad. After a betting round closes, the picker auto-opens on the next-to-deal board slot (ref-tracked sentinel keeps it from re-opening after the user dismisses). River close auto-advances to showdown. Desktop UI is byte-identical above 640px; tablet (640-1024) reuses desktop.
...
- `components/poker/Recorder.tsx` — Inline components: `NumericInput` (uncontrolled, blank-while-typing, blur-commits-or-zero), `Kbd`, `StreetPill`, `MetaStrip` (locks to read-only outside `setup`), `SetupPopover` (Game chips at top: NLHE / PLO4 / PLO5; Players / Blinds [disabled in bomb pot] / Straddle [disabled in bomb pot] / Bomb pot / Double board / Stack + Apply to all / Hero position [setup-only] / Save & Load default footer), `ActionBar` (`maxAllowed = isPotLimit(gameType) ? min(allIn, lastBet + (pot + toCall)) : allIn`; chips are `½/⅔/Pot/1.25×/All-in` for NLHE and `½/⅔/Pot/Max` for PLO; slider hard-stops at `maxAllowed`), `ActionLog`. `resolvedAwards` memo dispatches to `awardPots` / `awardPotsMultiBoard` based on `state.doubleBoardOn`, threading `state.gameType`. **`saveHand` was extracted to `lib/recorder/save-hand.ts`** so the mobile recorder can share the same payload-build + equity-precompute + server-action pipeline.
- `components/poker/RecorderShell.tsx` — Client wrapper that branches `/record` between `Recorder` and `MobileRecorder` via `useIsMobile`. Mounted by `app/record/page.tsx` after the auth gate.
- `components/mobile/MobileRecorder.tsx` — Top-level mobile recorder. Reuses the engine reducer and all derivations; only the UI is parallel. Owns: state (reducer + setup/picker/sizer sheet open flags), auto-advance effects (street → next street, river → showdown, hand-over → showdown), picker auto-open effect (ref-tracked sentinel keyed on phase + slot + boardIdx), and the inline `SetupCTA`, `ShowdownPanel`, `ActionButton` sub-components.
- `components/mobile/VerticalFelt.tsx` — Portrait-ellipse felt component shared by recorder + (eventually) replayer. `bottomInset` prop reserves space for the replayer dock by lifting every seat upward. Local `MobileFannedCard`, `FannedCardBack`, `FannedSlot` for PLO; local `CardFace`, `CardBack`, `CardSlot` for NLHE. All sized off the shared `CARD_DIM` constant (sm 30×42 for villains, md 44×62 for hero) so NLHE / PLO stay visually consistent. Bet bubbles render in a separate absolute layer with `chipXY`-style center interpolation. `BoardRow` renders one or two stacked rows depending on `state.doubleBoardOn`.
- `components/mobile/CardPickerSheet.tsx` — Bottom-sheet card picker. Suit-then-rank selection with Unicode-glyph cards (must match engine's `K♠` format — letter suits silently rejected by the engine). Suit colors split into `SUIT_COLOR_ON_DARK` (suit row, dark sheet bg) and `SUIT_COLOR_ON_LIGHT` (rank grid, white card bg) so spades stay visible on both. Direct-pick model — tap a rank, picker dismisses or `MobileRecorder` reopens it on the next slot.
- `components/mobile/RaiseSizer.tsx` — Bet/raise sizer with two presentations branched on viewport height: `ModalSizer` at vh<700, `DrawerSizer` at vh≥700. Shared `useSizerControls` hook holds the chip set, slider clamp, and pot-limit cap (mirrors the desktop `ActionBar` math). Big amount is a typeable `inputMode="numeric"` input that auto-clamps to `maxAllowed`. Slider, chip presets, and typed input all write to the same `amount` state.
- `components/mobile/SetupSheet.tsx` — Bottom-sheet replacement for the desktop `SetupPopover`. Same controls (game type, players, blinds, straddle, bomb pot, double board, default stack, hero position) plus Save/Load defaults footer. Inputs use the same uncontrolled blur-commit pattern as the desktop `NumericInput`. Hero position chips render only during setup phase (matches desktop convention).
- `components/ui/bottom-sheet.tsx` — Generic `<BottomSheet>` primitive used by `CardPickerSheet` and `SetupSheet`. Slide-up 280ms cubic-bezier, tap-backdrop / drag-grabber-60pt / Esc to dismiss, content-height with optional 85vh cap, iOS safe-area-inset-bottom padding, scroll-lock while open. Mount uses setState-during-render with sentinel; unmount is delayed via setTimeout so the slide-down animation plays — both patterns sidestep `react-hooks/set-state-in-effect`.
- `lib/use-is-mobile.ts` — `useIsMobile()` hook returning `{ isMobile, isTablet, isDesktop, bp }`. Built on `useSyncExternalStore` subscribed to **both** `window resize` and `matchMedia` change events (DevTools' device toolbar sometimes doesn't fire `resize` when swapping presets — `matchMedia` is the reliable trigger). Server snapshot resolves to `"desktop"` so SSR matches the existing desktop UI; client swaps after hydration. Breakpoints: `<640` mobile, `640-1024` tablet, `≥1024` desktop.
- `lib/recorder/save-hand.ts` — Extracted `buildAndSaveHand({ state, resolvedAwards, heroPos, heroPosName })` pipeline shared by both desktop `Recorder` and `MobileRecorder`. Builds the `SavedHand` payload, yields a frame so the calling transition paints "Saving…" before the equity precompute locks the main thread, runs `precomputeEquityByStep` for board1 (and board2 in double-board mode), POSTs via `saveHandAction`, returns the new hand id. No React, no router, no alerts — call sites wrap in their own `useTransition` + navigation.
...
(Empty — the mobile recorder ships the previously-blocking responsive pass for `/record`. Remaining mobile pages do not block real users since the existing desktop UI is at least readable on phones; see the next section for the rollout plan.)

## Mobile redesign — in progress

Multi-PR rollout living on the `dev` branch. Design specs are in `design_handoff_mobile_redesign/` (recorder / replayer / dashboard) and `design_handoff_landing_page/` (landing). All scope ships to `dev` first, gets reviewed via the Vercel preview attached to `dev`, and merges to `main` only when the whole mobile pass is done. Architecture: sibling components under `components/mobile/`, branched at the route via `useIsMobile` (≥640 → existing desktop UI, <640 → mobile sibling). Tablet (640-1024) reuses desktop. State lives in the existing engine reducer + helpers — only the UI is parallel.

### Shipped
- ✅ **Foundation** — `lib/use-is-mobile.ts`, `components/ui/bottom-sheet.tsx`, mobile design tokens in `app/globals.css`. See "Code map" entries.
- ✅ **Mobile recorder** (`/record`) — described in "What's working" above. The `saveHand` pipeline is extracted to `lib/recorder/save-hand.ts` so future surfaces can share the build + precompute + persist sequence.

### Remaining

In suggested order — each is a separate PR so the user can review on the Vercel preview before the next one starts.

1. **Mobile dashboard** (`/dashboard` below 640px). Card-per-hand list (one card per saved hand with hero hole cards, name, stake, date, result, star). Sticky search bar + filter pill at top, filter sheet (bottom sheet) with the existing dashboard's filters (Stakes / Position / Game / Pot type / Tags / Date range / Result sign). Long-press to enter select mode (header morphs to "N selected" with cancel + overflow). FAB bottom-right linking to `/record`. Spec: `design_handoff_mobile_redesign/README.md` "Screen 3". Pattern: build `components/mobile/MobileDashboard.tsx`, branch at route via a `DashboardShell` similar to `RecorderShell`, reuse the existing `Dashboard` data fetch + server actions verbatim.
2. **Mobile replayer** (`/hand/[id]` below 640px). Same `VerticalFelt` as the recorder but `bottomInset={96}` (lifts seats so the dock can't occlude the hero plate). Persistent floating dock at the bottom (transport buttons, scrubber with annotation dots, step counter). Annotation balloon slides up from the dock when the scrubber lands on an annotated step. Anonymous-viewer CTA pill (sticky below header, dismissible via cookie). Owner-only Notes/Venue/Date strip is **deferred to v1.1** per the brief. Spec: `design_handoff_mobile_redesign/README.md` "Screen 2". Pattern: build `components/mobile/MobileReplayer.tsx` + `ReplayDock.tsx` + `AnnotationBalloon.tsx`, branch at route via a `ReplayerShell`.
3. **Auth pages** (`/login`, `/signup`) — out of scope for the mobile recorder PR but called out in v1.1 backlog (item 4). Bring them into the neutral charcoal + Inter system; while you're in there, make them mobile-friendly (the layouts are simple enough that a single responsive pass can cover both). The OAuth buttons in `components/auth/OAuthButtons.tsx` already lay out fine on narrow viewports; the form chrome is what needs work.
4. **Landing page redesign** — separate scope from the mobile rollout. New design lives in `design_handoff_landing_page/` (sample hand embedded, simpler structure). The existing `components/landing/` directory is the target. The new app-icon font in that handoff is also called out by the user as a future polish item to apply across the favicon / OG / Twitter / manifest assets.

### Open questions deferred from the brief
- 7+ handed seat coordinates beyond 8 — capped at 8-handed for the mobile felt; a "+1 seat" pill at 9-handed would be the next step if anyone records 9-handed live games.
- Owner-only Notes/Venue/Date strip on the mobile replayer — punted from v1, mirror the desktop pattern when shipping the mobile replayer if user feedback asks for it.
- Landscape orientation on mobile — auto-rotates the existing tablet layout for now; a tuned landscape recorder layout is open if usage tells us it matters.
...
3. **Auth-page styling** — `/login` and `/signup` are still default-ish. Bring them into the neutral charcoal + Inter system. (Tied to the mobile rollout — see "Mobile redesign — in progress" item 3.)
4. **Bet-sizing presets aware of blinds** — at 1/2 stakes, opens are usually 5-10. A `2.5×` / `3×` / `pot + 1bb per limper` chip would cut entry time. The drawer's chip array is already trivially extensible (`chips: { id, label, amount }[]`); the PLO branch already takes a different chip set, so per-game chip overrides are wired. The mobile `RaiseSizer` shares this chip-set shape via the inline `chips` array in `useSizerControls`.
5. **Edit gameplay actions on a saved hand.** Today only metadata (name / venue / date / notes / tags / fav) is editable. Re-recording is the workaround.
...
8. **Per-hand OG image** — `/hand/[id]` social-share unfurl currently uses the generic `app/opengraph-image.tsx`. A per-route variant could show the specific hand's name + board cards in the unfurl. New `app/hand/[id]/opengraph-image.tsx` reading from `getHandForViewing`.
9. **Per-route titles** — `/record` → "Record", `/dashboard` → "Dashboard", `/hand/[id]` → the hand name. The `metadata.title.template` is already wired (`%s — savemyhands`); each route just needs `export const metadata = { title: "..." }`.
10. **`robots.ts` / `sitemap.ts`** — public hands as a sitemap → SEO. Not a v1 blocker.
11. **Player-name autocomplete across hands** — record "Whale" once → suggest at the same venue.
12. **Toast primitive** — six `window.alert` calls remain (auth error, save / delete / update fail, share partial fail, notes fail). The mobile recorder also keeps a couple. Build a non-modal toast (similar shape to `confirm-dialog`'s provider + hook) to replace them.
13. **Single-row "Add tag" still uses `window.prompt`** — there's a `TagPopover` already wired for the bulk action bar; the row-level Add tag button can re-use it. Smaller win since the per-row tag flow is rare.
14. **Sweep remaining warm `oklch(... 60)` arbitrary classes** in `Recorder.tsx` / `Replayer.tsx` for full neutral parity. Mechanical.
15. **Apply new app-icon font across branding assets** — favicon, OG image, Twitter card, manifest, Apple touch icon. The new font lives in the `design_handoff_landing_page/` package; user wants it applied as a future polish pass.

### Done since 2026-05-15
- ✅ Mobile redesign foundation: `--smh-mobile-*` tokens in `app/globals.css`, `lib/use-is-mobile.ts` (matchMedia + resize subscription, SSR-safe), generic `<BottomSheet>` primitive in `components/ui/`.
- ✅ Mobile recorder shipped end-to-end: `RecorderShell` route branch, `MobileRecorder` orchestration (reuses engine reducer + all derivations), `VerticalFelt` portrait-ellipse table with hand-tuned 6/7/8-handed seat coords, `CardPickerSheet` (suit-then-rank, Unicode glyphs matching engine format), `RaiseSizer` (modal at vh<700, drawer at vh≥700, typeable numpad input), `SetupSheet` (replaces desktop popover), inline `ShowdownPanel` with per-villain Show/Muck and villain-card picker.
- ✅ `saveHand` extracted from `Recorder.tsx` into `lib/recorder/save-hand.ts` so desktop and mobile recorders share the same build + equity-precompute + persist pipeline. Pure refactor, no behavior change.
- ✅ Bet bubbles on the mobile felt interpolate toward the felt center via the same `chipXY`-style math the desktop uses (separate absolute layer; not stacked under each plate).
- ✅ Mobile felt card sizing unified across NLHE / PLO (`CARD_DIM` constant) so 8-handed PLO5 side seats fit inside the felt edge and NLHE villain backs are no longer postage stamps.
- ✅ Picker auto-opens on the next-to-deal board slot when a betting round closes (ref-tracked sentinel keyed on phase + slot + boardIdx prevents re-open after dismiss). River close auto-advances to showdown.
...
- **Card-id format is rank + Unicode-glyph suit (`"K♠"`)** everywhere — engine, equity, hand-eval, picker, felt. Mobile's `CardPickerSheet` originally stored letter suits (`"Ks"`) and the engine silently rejected every pick; the bug presented as cards never appearing on the felt. If you build a new picker call site, mirror the existing pattern (`SUITS = ["♠", "♥", "♦", "♣"]`).
- **Spade glyph color depends on background.** On dark surfaces (suit row in the picker) use a near-white tint so it's visible; on white card faces use near-black. `CardPickerSheet`'s `SUIT_COLOR_ON_DARK` / `SUIT_COLOR_ON_LIGHT` split is the canonical example. A single shared color collapsed spades to white-on-white the first time around.
- **`useIsMobile` subscribes to BOTH `window resize` and `matchMedia` change events.** Chrome DevTools' device toolbar sometimes doesn't fire `resize` when swapping presets; `matchMedia` fires reliably. Don't drop one in favor of the other.
- **`<BottomSheet>` mount/unmount uses setState-during-render with sentinel + delayed-unmount via setTimeout.** This sidesteps `react-hooks/set-state-in-effect` (which blocks the obvious `useEffect(() => setShouldRender(true), [open])` pattern) while still rendering during the slide-down animation.
- **Mobile and desktop recorders share state ONLY via the engine reducer.** They're sibling components with their own React trees — there's no shared component instance and no in-memory state crossover. The shared surface is `lib/recorder/save-hand.ts` (save pipeline) and `components/poker/engine.ts` (reducer + derivations). If you find yourself wanting to share UI logic, consider extracting a hook rather than a component.
- **Default villains have `cards: null`** (per `defaultPlayers` in `engine.ts`). Anything that derives "expected hole-card count" from `villain.cards.length` is a bug — use `holeCardCount(state.gameType)` instead. The mobile recorder hit this twice (PLO villain backs rendered as 2 cards instead of 4/5; villain showdown picker auto-advance closed after the first card because it scanned a length-0 array).
- **Mobile felt's bet bubbles render in a separate absolute layer**, not inline under each seat plate. Each bubble's `(left, top)` is a linear interpolation from the seat's coords toward `(50, 50)` by a per-seat `t` factor (0.45 for hero — clears the hole-card row; 0.30 elsewhere). Same idea as desktop's `chipXY` in `components/poker/lib.ts`.
- **Mobile felt's picker auto-open uses a `useRef` sentinel** keyed on `(phase, slot, boardIdx)` so dismissing the picker doesn't trigger an immediate re-open. Manual taps on a board slot still work — they go through `openBoardSlot` directly, not the auto-open effect.
- **Mobile recorder hides settings access during play.** The gear button only appears in the meta strip during `phase === "setup"`; once the hand starts the slot shows the street pill and settings are locked (matches the existing desktop convention that `SetupPopover` goes read-only outside setup).
- **Picker / felt suit colors are duplicated between `CardPickerSheet` and `VerticalFelt`** (small color-by-suit lookups). Both must match the engine's Unicode glyphs. If you change the suit palette, change both.
- **Mobile fanned cards use `MobileFannedCard` (local to `VerticalFelt.tsx`)**, not the shared `components/poker/FannedPlayingCard.tsx`. The shared component is sized for desktop; the mobile copy uses smaller `CARD_DIM` / `FANNED_CARD` constants tuned so 5-card PLO5 fans on 8-handed side seats don't run off the felt edge. Don't bump the shared component's sm/md to "fix" mobile — it'll break the desktop replayer.
- **Vercel preview is attached to `dev`** — push to `dev` and the preview rebuilds automatically. The mobile redesign all lands there before merging to `main`.
